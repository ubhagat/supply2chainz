...
for k, v in dict.iteritems():
    ...
    if lenv > 1:
        ...
        for alpha in range(len(values_at_each_state)):
            shipDataTemp = [shipNumber, v[0][2], ipdate, zipcode]
        ...
        shipData.append([shipNumber, row[2], ipdate, zipcode, row[11], row[5], 1, 0, row[11], row[10]])
        shipMap.append([shipNumber, row[0], row[10], row[9]])
        shipNumber += 1

with open("zip_shipData.csv", 'w') as outfile:
    writer = csv.writer(outfile, delimiter = ',')
    writer.writerows(shipData)

with open("zip_shipMap.csv", 'w') as outfile:
    writer = csv.writer(outfile, delimiter = ',')
